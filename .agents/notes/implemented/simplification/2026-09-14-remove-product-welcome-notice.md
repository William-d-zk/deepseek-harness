...
Its copy spoke as the framework vendor. Both language bodies named DeepSeek Harness, described its `0.1` release as a stage of testing for Harness developers, and invited readers into the DSH plugin ecosystem. A deployment that ships this console as its own product cannot restate that: it is not in a `0.1` developer-testing stage of the framework's making, and its users are not plugin authors. The copy lived in `packages/client/ui-settings-models/src/onboarding-copy.ts`, and the strings shipped as the `welcomeTitle`, `welcomeBody`, `welcomeContinue`, and `welcomeError` keys in both locales.
