...
所需的客户端符号在测试侧**镜像**了一份（`scaffold.ts` 曾导出镜像后的 welcome-notice 常量，随着[产品内测声明移除](../simplification/2026-09-14-remove-product-welcome-notice.zh.md)一同删除；两个 chat e2e 直接引 `dsh-client-runtime/client` 因为 `runtime` 工程本来就在 host 图里），从而让那 4 个消费者离开了 host 图；`apps/cli/tsconfig.json` 里 15 条 client 工程引用随之失去 owner-map 职责，已一并删除。镜像值与源逐字一致，漂移的表现是选择器失配或镜像值陈旧，都是响亮失败。
