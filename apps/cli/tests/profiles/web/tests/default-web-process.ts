...
/**
 * The Vitest case surface this helper drives. Stated structurally because the
 * `apps/web` lane resolves the vitest copy bound to its own vite 6 dependency
 * while this module resolves the root copy: the two copies are the same version
 * but their `TestContext` types are unrelated, so a `TestContext` parameter
 * rejects the caller's case.
 */
interface DefaultWebCase {
 ...
 */
export async function withDefaultWeb(test: DefaultWebCase, inspect: (app: DefaultWeb) => Promise<void>): Promise<void> {
