...
/** One mounted guest webview with its scripted navigation and lifecycle events. */
export interface ElectronFixtureGuest {
  readonly element: WebviewElement
  readonly state: { url: string; title: string; loading: boolean; back: boolean; forward: boolean }
  readonly emit: (type: string, fields?: object) => void
  readonly loadURL: Mock
  readonly getURL: Mock
  readonly getTitle: Mock
  readonly canGoBack: () => boolean
  readonly canGoForward: () => boolean
  readonly clearHistory: Mock
  readonly goBack: Mock
  readonly goForward: Mock
  readonly reload: Mock
  readonly isLoading: () => boolean
}

/** The native bridge one fixture installs, with each operation left observable. */
export interface ElectronFixtureBridge extends DesktopBrowserBridge {
  readonly acquire: Mock
  readonly release: Mock
  readonly onOpenRequested: Mock
}

/** One isolated Electron page whose native operations are spies the test drives. */
export interface ElectronFixture extends BrowserPage {
  readonly presentation: ElectronWebviewPresentation
  readonly bridge: ElectronFixtureBridge
  readonly workspace: Mock
  readonly persist: Mock
  readonly openRequested: Mock
  readonly opens: ReadonlySet<(url: string) => void>
  readonly guests: readonly ElectronFixtureGuest[]
  readonly host: HTMLDivElement
  readonly reservation: DesktopBrowserReservation
  /** @returns ends physical attachment; a later mount may reload the page. */
  mount(): () => void
  /** @returns the guest webview the last approved reservation attached. */
  guest(): Promise<ElectronFixtureGuest>
  /** @returns completion of frame disposal and fixture cleanup. */
  dispose(): Promise<void>
}

/** @returns one isolated, explicitly mounted page with native operations replaced by spies. */
export function electronFixture(initial?: BrowserTabState): ElectronFixture {
