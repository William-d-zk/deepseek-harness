...
describe('scope tree', () => {
  it('publishes transient Assistant chunks and the named durable v2 settlement through one event source', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 's1' }])
...
describe('catalog-addressed navigation', () => {
  it('opens an explicit catalog without retaining it and keeps one-shot labels optional', async ({ world }) => {
    const b = world()
    b.svc.handleControlFrame({
...
describe('fork', () => {
  ...
  it('floors a fractional anchor to the real event seq the wire accepts', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 'source', cwd: '/work' }])
    b.mock.remote.session.fork.mockResolvedValue(ok({ sessionId: sid('child') }))
  ...
  it('rejects child rename failure while preserving its catalog row and releasing the operation', async ({ world }) => {
    const b = world()
    b.svc.handleControlFrame({
...
describe('catalog arrival', () => {
  it('keeps a retained generation indexed after its Host row is removed', async ({ world }) => {
    const b = world()
