...
const it = createClientTest({ roster: API_ROSTER }).extend<{ world: BenchFactory }>({
  world: async ({ mock, start }, use) => {
...
describe('list store projection', () => {
  it('projects durable titles separately from cwd/id display fallbacks and parent links', async ({ world }) => {
    const b = world()
  ...
  it('reprojects a blank session from the generic agent-preset projection', async ({ world }) => {
    const b = world()
  ...
  it('reflects live increments (host stream via manager) into the store', async ({ world }) => {
    const b = world()
...
describe('search', () => {
  it('delegates transient content search without changing the list snapshot', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 's1' }])
...
describe('scope tree', () => {
  it('opens a conversation from follow projections without a second projection request', async ({ world }) => {
    const b = world()
  ...
  it('publishes the final Assistant message before retiring transient chunks at Step end', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 's1' }])
  ...
  it('replaces an active assistant baseline on reconnect without duplicate chunks', async ({ world }) => {
    const b = world()
  ...
  it('stages a post-opening assistant settlement behind its exact active attempt', async ({ world }) => {
    const b = world()
  ...
  it('replaces an invalid settlement with the authoritative post-end baseline', async ({ world }) => {
    const b = world()
  ...
  it('holds a Host-addressed Context across an empty catalog baseline', async ({ world }) => {
    const b = world()
  ...
  it('borrows only retained bindings and preserves them while the catalog changes', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 's1' }])
  ...
  it('closes an opened journal when its removed scope drops', async ({ world }) => {
    const b = world()
...
describe('Agent scope disposal lifecycle', () => {
  it('root disposal runs Agent scope effects', async ({ world }) => {
    const b = world()
  ...
  it('root disposal waits for an opened Session source to finish closing', async ({ world }) => {
  ...
  it('root disposal joins every Session drop already started by final release under load', async ({ world }) => {
...
describe('borrow-only bindings', () => {
  it('keeps catalog discovery separate from history opening and ownership', async ({ world }) => {
    const b = world()
...
describe('catalog-addressed navigation', () => {
  it('retains a projected child independently of its parent and shares its history generation', async ({ world }) => {
    const b = world()
  ...
  for (const title of [undefined, 'Investigate startup']) {
    it(`loads an unretained one-shot child with projected title ${String(title)}`, async ({ world }) => {
      const b = world()
  ...
  it('keeps projected titles in standard list rows for an addressed route', async ({ world }) => {
    const b = world()
  ...
  it('projects a retained descendant and discovers ancestor addresses without retaining ancestor scopes', async ({ world }) => {
    const b = world()
    b.mock.remote.session.projections.mockImplementation((payload) => {
      const { sessionId } = payload as { sessionId: SessionId }
      if (sessionId === sid('root')) {
...
describe('create', () => {
  it('passes a preallocated id and preserves it on ordinary failure', async ({ world }) => {
    const b = world()
  ...
  it('publishes a created identity without implicitly retaining its binding', async ({ world }) => {
    const b = world()
  ...
  it('lists the published id after Workspace attachment fails (publication precedes attachment)', async ({ world }) => {
    const b = world()
...
describe('fork', () => {
  it('propagates a failed fork without creating or retaining a child', async ({ world }) => {
    const b = world()
  ...
  it.for([
    ...
    ['计划（1）', '计划（2）'],
    ['计划 （9）', '计划 （10）'],
  ] as const)('increments the durable title %j after the child is published', async ([sourceTitle, childTitle], { world }) => {
    const b = world()
    b.svc.handleControlFrame({
  ...
  it('sends the exact boundary seq verbatim: callers pass real event seqs', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 'source', cwd: '/work' }])
    b.mock.remote.session.fork.mockResolvedValue(ok({ sessionId: sid('child') }))
  ...
  it('does not rename without the title policy or a durable source title', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 'source', cwd: '/work' }])
    b.mock.remote.session.fork.mockResolvedValue(ok({ sessionId: sid('child') }))
  ...
  it('rejects child rename failure while preserving its catalog row without retaining the child', async ({ world }) => {
    const b = world()
    b.svc.handleControlFrame({
...
describe('catalog arrival', () => {
  it('keeps a retained binding alive without a catalog row after Host removal', async ({ world }) => {
    const b = world()
...
describe('blank mirror', () => {
  it('flips blank=false from the running:true status frame (cross-client conversion)', async ({ world }) => {
    const b = world()
  ...
  it('flips blank=false on prompt ACCEPTANCE, not on the attempt', async ({ world }) => {
    const b = world()
  ...
  it('keeps a rejected first prompt blank: hidden and still reusable', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 's1', blank: true, cwd: '/w/a' }])
    using reference = b.svc.retain(sid('s1'), { source: 'controllerOperation' })
    await reference.ready
  ...
  it('takes session-added blank=true as the hidden birth and list blank as reconnect authority', async ({ world }) => {
    const b = world()
    await feedList(b, [])
    b.svc.handleSessionAdded({ agentAvailable: true,
  ...
  it('never re-blanks: a stale blank=true summary cannot hide an engaged session', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 's1', blank: true }])
    using reference = b.svc.retain(sid('s1'), { source: 'controllerOperation' })
    await reference.ready
...
describe('coverage tails (branch duals)', () => {
  it('displayTitleOf falls back to the id for empty and separator-only cwd', async ({ world }) => {
    const b = world()
  ...
  it('reading an unknown binding leaves an existing reference unchanged', async ({ world }) => {
    const b = world()
    await feedList(b, [{ id: 's1' }])
    using reference = b.svc.retain(sid('s1'), { source: 'controllerOperation' })
    await reference.ready
