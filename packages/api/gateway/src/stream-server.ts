...
export class RemoteStreamMuxServer {
   ...
   * @param account - Host-login account the upgrade's cookie resolved to, or
   *   null; every stream opened on this socket runs under it.
   */
  handleUpgrade(req: IncomingMessage, socket: Duplex, head: Buffer, peer: PeerScope, account: string | null = null): void {
    this.server.handleUpgrade(req, socket, head, (websocket) => {
      ...
      const connection = new RemoteStreamMuxConnection(websocket, bound, this.failure, this.streamInboxBytes, account)
