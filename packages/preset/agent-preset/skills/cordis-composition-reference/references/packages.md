...
| `@deepseek-ai/dsh-client-ui-settings-general` | no | Settings ownerless-copy and product onboarding plugin: the General section, shell trigger/header chrome content, settings dictionaries |
...
| `@deepseek-ai/dsh-page-feedback` | yes | Visual page-annotation feedback loop: on-page overlay capture, a persistent pending/acknowledged/resolved/dismissed store with audit events, long-poll watch, and an ego-browser resolve-verification seam — for human-driven frontend debugging |
